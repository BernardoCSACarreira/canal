...
|---|---|---|---|---|
| B1 | **Decided and exercised** — [ADR 0032](decisions/0032-wal-format-versioning.md): the WAL container is versioned, a build reads one step back and migrates by rewrite at Open. Format version 2 shipped as the first exercise: a delete record carries its epoch, orphaned floors survive compaction, a hand-pinned V1 fixture proves the upgrade, and both decoders are fuzzed | the door is held AND walked through once: a format bump no longer strands a log, demonstrably | [wal.go](../pkg/store/wal/wal.go), [format.go](../pkg/store/wal/format.go) | unblocked: M1's store work; the delete-floor gap in the revocation fence is closed |
