...
**TypeScript (Phase 1 combined scaffold):** `services/ingestion-api/src/control/read-models.ts` and `src/routes/control.ts` — still serves these paths in the default single-service layout; keep responses aligned with Python until a gateway demotes duplicate routes.
