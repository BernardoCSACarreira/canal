...
//
// IT DOES NOT DEMONSTRATE THE FENCE, and that is now a statement about THIS fixture rather than
// about the code. Deleting commitAllowed and all three of the ledger's revoked checks leaves this
// passing: with one lane, revoking it is also what stops the pipeline, so the read loop has ended
// and the run is draining before the held records settle. No acknowledgement is generated by any
// path, and a fence has nothing to stop.
//
// TestRecordsSettlingAfterARevocationNeverReachTheUpstream in revocation_test.go is the fixture that
// does demonstrate it: two lanes, one revoked, a sink that defers durability instead of blocking, so
// the run stays alive and both lanes settle together. Deleting all three fences fails THAT one.
//
// This is kept because it asserts the same property one layer down and much more cheaply, and
// because a single-lane revocation is its own case. Honestly labelled rather than deleted.
